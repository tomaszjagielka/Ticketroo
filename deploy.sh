...
pm2 restart $BACKEND_ENTRY --name ticketroo-backend --update-env || pm2 start $BACKEND_ENTRY --name ticketroo-backend --update-env --env PORT=$BACKEND_PORT
...
if curl -s http://localhost:$BACKEND_PORT/status; then
